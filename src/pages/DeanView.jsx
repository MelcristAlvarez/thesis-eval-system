 ...
 * Dean can see Chair evaluation STATUS (Done/Pending), but NOT the Chair's raw scores before AI synthesis.
...
/* ── Dean evaluation criteria (CHED standard) ── */
const deanCriteria = [
    {
...
function Overview() {
    ...
    // Fixed: Now checks for Dean's evaluation status
                    ...
                    <table style={{ width: "100%", borderCollapse: "collapse" }}>
                        <thead>
                            <tr style={{ background: "var(--bg-base)" }}>
                                {["Faculty", "Subject / Code", "Student", "Chair Eval", "Dean", "Composite", "Status"].map(h => (
                                    <th key={h} style={{
                        ...
                        <tbody>
                            {facultyList.map((f, i) => (
                                <tr key={f.id}
                                    ...
                                    {/* Chair Status (Dean only sees if it's done, not the score) */}
                                    <td style={{ padding: "12px 14px" }}>
                                        {f.chairEvaluated
                                    ...
                                    <td style={{ padding: "12px 14px", fontFamily: "var(--font-display)", fontSize: "16px", fontWeight: 600 }}>
                                        {f.deanEvaluated ? Number(f.deanScore).toFixed(2) : <span style={{ color: "var(--text-muted)", fontSize: "12px" }}>—</span>}
                                    </td>
...
function DeanEvalTab() {
    const [selected, setSelected] = useState(null);
    const [ratings, setRatings] = useState({});
    const [remarks, setRemarks] = useState("");
    const [submitted, setSubmitted] = useState(new Set(
        facultyList.filter(f => f.deanEvaluated).map(f => f.id)
    ));
    const [done, setDone] = useState(false);

    const pending = facultyList.filter(f => !submitted.has(f.id));
    const allRated = deanCriteria.every(c => ratings[c.id]);
    ...
    return (
        <div className="anim-fade-in" style={{ maxWidth: "680px", margin: "0 auto" }}>
            ...
            <Card>
                ...
                {/* Criteria */}
                {deanCriteria.map((c, ci) => (
                    <div key={c.id} style={{
                        marginBottom: "26px", paddingBottom: "26px",
                        borderBottom: ci < deanCriteria.length - 1 ? "1px solid var(--border)" : "none"
                    }}>
                ...
                <div style={{ marginBottom: "22px" }}>
                    ...
                    }}>Dean Remarks <span style={{ fontWeight: 400, textTransform: "none" }}>(Optional)</span></p>
...
export default function DeanView({ activeTab }) {
    ...
    if (activeTab === "evaluate") return <div className="anim-fade-up"><DeanEvalTab /></div>;
