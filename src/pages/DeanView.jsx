/**
 * DeanView.jsx
 * DEAN eval criteria mapped exactly to HRMO-Form 03.4A.1 and Classroom Observation Tool
...
function Overview({ facultyList }) {
  ...
  const pending = facultyList.filter(f=>!f.deanEvaluated).length;
  return (
    <div className="anim-fade-in">
      <div className="kpi-3" style={{ gap:"14px", marginBottom:"22px" }}>
        {[
          { label:"Total Faculty",       value:facultyList.length, sub:"in system"           },
          { label:"College Average Score", value:avg,                sub:"composite score"     },
          { label:"Pending Dean Evals", value:pending,            sub:"awaiting your input" },
        ].map((k,i)=>(
          <Card key={i}>
          ...
          <table style={{ width:"100%", borderCollapse:"collapse" }}>
            ...
            <tbody>
              {facultyList.map((f,i)=>(
                <tr key={f.id}
                  style={{ 
                      borderBottom:i<facultyList.length-1?"1px solid var(--border)":"none", 
...
function DeanEvalTab({ facultyList, preSelected, onBack, refreshData }) {
  ...
  const pending = facultyList.filter(f=>!f.deanEvaluated);
  ...
  const handleSubmit = () => {
    ...
    // Check text areas (Minimum 80 chars)
    ...
    // Strict 30/40/30 weighting calculation for final composite
    let totalWeight = 0;
    let weightedSum = 0;
    ...
    // Program Chair (40%)
    if (selectedFaculty.chairScore) {
        weightedSum += selectedFaculty.chairScore * 0.40;
        totalWeight += 0.40;
    }

    // Department Head / Dean (30%) - Current submission
    weightedSum += parseFloat(overallRating) * 0.30;
    totalWeight += 0.30;
    ...
    const updatedFaculty = {
        ...selectedFaculty,
        deanEvaluated: true,
        deanScore: parseFloat(overallRating),
  ...
  if (submitted && submitResult) return (
    <div className="anim-fade-in" style={{ textAlign:"center", padding:"40px 0" }}>
      ...
      <h3 style={{ fontSize:"18px", fontWeight:600, marginBottom:"8px" }}>Dean Evaluation Submitted</h3>
      ...
      {/* HRMO Calculation Breakdown Card */}
      <div style={{ maxWidth: "450px", margin: "0 auto 30px", background: "var(--bg-base)", border: "1px solid var(--border)", borderRadius: "12px", padding: "20px", textAlign: "left" }}>
         ...
         <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <span style={{ fontSize: "14px", fontWeight: 700, color: "var(--text-primary)" }}>Dean Overall Rating</span>
    ...
    <div className="anim-fade-in" style={{ maxWidth:"720px", margin:"0 auto" }}>
      ...
      <Card id="eval-item-remarks" style={{ 
        ...
        <p style={{ fontSize: "14px", fontWeight: 700, letterSpacing: "0.07em", textTransform: "uppercase", color: "var(--text-primary)", marginBottom: "12px" }}>
            Department Head / Dean Remarks
        </p>
        <p style={{ fontSize: "12px", color: "var(--text-muted)", lineHeight: 1.6, marginBottom: "16px" }}>
            Share specific observations regarding syllabus alignment, research engagement, and overall performance. Your comments are processed by the feedback engine to generate structured reports. (Minimum 80 characters required)
        </p>
        <textarea
            value={remarks}
            onChange={(e) => {
            ...
            placeholder="Add academic observations, college alignment, or specific areas for development..."
            rows={5}
            style={{
                width: "100%", padding: "16px", background: "var(--bg-input)",
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "24px" }}>
                    <div>
                        ...
                        <h2 style={{ fontSize: "22px", fontWeight: 700, color: "var(--text-primary)" }}>
                            {isStudent ? `Student Comment #${evidence.data.id}` : `${evidence.type === 'chair' ? 'Program Chair' : 'Department Head / Dean'} Evaluation Record`}
                        </h2>
...
export default function DeanView({ activeTab }) {
  ...
  if (activeTab === "evaluate") {
      if (evaluatingFaculty) {
          return <div className="anim-fade-up"><DeanEvalTab facultyList={localFacultyList} preSelected={evaluatingFaculty} onBack={() => setEvaluatingFaculty(null)} refreshData={refreshData} /></div>;
      }
      return <div className="anim-fade-up"><DeanEvalTab facultyList={localFacultyList} preSelected={null} onBack={()=>{}} refreshData={refreshData}/></div>;
  }
